...
There is no committed test suite. Prefer the lowest-impact checks that exercise
the changed surface:
...
Custom commands such as `query`, `ingest`, `status`, `scan`, and ensemble
...
- `ensembles`: ensemble metadata, physics parameters, configuration ranges,
  HMC paths, nicknames, tags, notes, and saved default params.
