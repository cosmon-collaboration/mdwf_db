...
#SBATCH -o /global/u2/s/smithwya/test_run/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48/mres_mq/jlog/%j.log

cd "/global/u2/s/smithwya/test_run/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48/mres_mq"
mkdir -p DATA
...
# Record job start
mdwf_db update -e 1 -o WIT_MRES_MQ -s RUNNING \
  -p "slurm_job_id=$SLURM_JOB_ID config_start=0 config_end=12 config_increment=4 wit_input=/global/u2/s/smithwya/test_run/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48/mres_mq/DWF_mres_mq.in workdir=/global/u2/s/smithwya/test_run/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48/mres_mq nodes=1 ranks=4" \
  || true
...
WIT_INPUT="/global/u2/s/smithwya/test_run/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48/mres_mq/DWF_mres_mq.in"

echo "Running mres-mq range 0-12 step 4"
...
if [ $EXIT_CODE -eq 0 ]; then
    ...
    mdwf_db update -e 1 -o WIT_MRES_MQ -s COMPLETED \
    ...
    mdwf_db update -e 1 -o WIT_MRES_MQ -s CANCELED \
      -p "slurm_job_id=$SLURM_JOB_ID exit_code=$EXIT_CODE runtime=$SECONDS host=$(hostname)" \
      || true
    exit $EXIT_CODE
else
    # Job failed
    mdwf_db update -e 1 -o WIT_MRES_MQ -s FAILED \
      -p "slurm_job_id=$SLURM_JOB_ID exit_code=$EXIT_CODE runtime=$SECONDS host=$(hostname)" \
      || true
    exit $EXIT_CODE
fi
