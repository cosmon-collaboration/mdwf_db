...
phase_1_init() {
    ...
    if mdwf_db init-db --base-dir="$TEST_DIR" --db-file="$MDWF_DB_URL" 2>&1 | tee /tmp/init_output.log; then
...
phase_2_ensemble_mgmt() {
    ...
    if mdwf_db query --db-file="$MDWF_DB_URL" --detailed 2>&1 | grep -q "beta"; then
    ...
    if mdwf_db query --db-file="$MDWF_DB_URL" -e "$ENSEMBLE_ID" 2>&1 | grep -q "beta.*6.0"; then
    ...
    if mdwf_db nickname --db-file="$MDWF_DB_URL" -e "$ENSEMBLE_ID" --set test_ens 2>&1 | grep -q "Nickname"; then
    ...
    if mdwf_db query --db-file="$MDWF_DB_URL" -e test_ens 2>&1 | grep -q "beta"; then
    ...
    if mdwf_db promote-ensemble --db-file="$MDWF_DB_URL" -e test_ens 2>&1 | grep -qE "(Promoted|PRODUCTION)"; then
    ...
    if mdwf_db scan --db-file="$MDWF_DB_URL" 2>&1 | grep -qE "(Scanned|configurations|Updated)"; then
...
phase_3_input_files() {
    ...
    if mdwf_db hmc-xml --db-file="$MDWF_DB_URL" -e test_ens \
        -o "$hmc_xml" \
    ...
    if mdwf_db glu-input --db-file="$MDWF_DB_URL" -e test_ens \
        -o "$glu_input" \
    ...
    if mdwf_db wit-input --db-file="$MDWF_DB_URL" -e test_ens \
        -o "$wit_input" \
        -i "Configurations.first=0 Configurations.last=100 Configurations.step=4" 2>&1; then
        check_file "$wit_input" "Configurations.first 0"
        check_file "$wit_input" "Configurations.last 100"
        check_file "$wit_input" "Lattice_parameters.Ls 12"
...
phase_4_slurm_scripts() {
    ...
    if mdwf_db hmc-script gpu --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db hmc-script cpu --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db smear-script --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db wflow-script --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db mres-script --db-file="$MDWF_DB_URL" -e test_ens \
        -j "nodes=1 gpus=4" \
    ...
    if mdwf_db mres-mq-script --db-file="$MDWF_DB_URL" -e test_ens \
        -j "nodes=1" \
    ...
    if mdwf_db meson2pt-script --db-file="$MDWF_DB_URL" -e test_ens \
        -j "nodes=1" \
    ...
    if mdwf_db zv-script --db-file="$MDWF_DB_URL" -e test_ens \
        -j "nodes=1" \
...
phase_5_operations() {
    ...
    if mdwf_db update --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db query --db-file="$MDWF_DB_URL" -e test_ens 2>&1 | grep -qE "(TEST_OP|operation)"; then
    ...
    if mdwf_db clear-history --db-file="$MDWF_DB_URL" -e test_ens 2>&1 | grep -qE "(Cleared|removed)"; then
...
phase_6_defaults() {
    ...
    if mdwf_db default_params set --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db default_params show --db-file="$MDWF_DB_URL" -e test_ens \
    ...
    if mdwf_db default_params delete --db-file="$MDWF_DB_URL" -e test_ens \
...
phase_7_cleanup() {
    ...
    if mdwf_db remove-ensemble --db-file="$MDWF_DB_URL" -e test_ens 2>&1 | grep -qE "(Removed|Deleted)"; then
