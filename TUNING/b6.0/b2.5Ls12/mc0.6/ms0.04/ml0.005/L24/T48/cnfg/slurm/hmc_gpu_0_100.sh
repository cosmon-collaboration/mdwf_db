...
echo "ens = $ens"
echo "ens_dir = /global/u2/s/smithwya/mdwf_db/TUNING/b6.0/b2.5Ls12/mc0.6/ms0.04/ml0.005/L24/T48"
...
mdwf_db update -e 1 -o HMC_tepid -s RUNNING \
  -p "slurm_job_id=$SLURM_JOB_ID n_trajec=$n_trajec traj_length=0.75 mode=tepid xml_path=$work_root/cnfg/HMCparameters.xml workdir=$work_root/cnfg nodes=1 config_start=$start config_end=$EC" \
  || true
