...
def generate_mres_mq_sbatch(
    *,
    ...
    """
    Create a mres-mq SBATCH script under ensemble_dir/mres_mq/
    """
    # mail_user optional
    ...
    if mc_final <= 0:
        raise ValueError(f"Invalid quark masses: mc={mc_final}")
    ...
    if not output_file:
        output_file = str(sbatch_dir / f"mres_mq_{config_start}_{config_end}_{config_inc}.sh")
...
EXEC="{exec_path}"
BIND="{bind_sh}"
echo "Running mres-mq range {config_start}-{config_end} step {config_inc}"
