...
DEFAULT_GLU_EXEC = (
    "/global/cfs/cdirs/m2986/cosmon/mdwf/software/install/GLU_ICC/bin/GLU"
)
...
def resolve_param_aliases(
    params: Dict[str, Any], schema: List[ContextParam]
) -> Dict[str, Any]:
...
def storable_params(schema: Optional[List[ContextParam]]) -> List[ContextParam]:
    """Return only the storable parameters from a schema."""
    if not schema:
        return []
    return [p for p in schema if p.storable]
...
def common_wit_gpu_params() -> List[ContextParam]:
    ...
    return [
        *common_slurm_params(),
        ...
        ContextParam(
            "run_dir", str, help="Working directory (defaults to ensemble directory)",
            storable=False
        ),
        ContextParam(
            "conda_env", str, default=DEFAULT_CONDA_ENV, help="Conda environment path"
        ),
        ContextParam(
            "bind_script", str, default=DEFAULT_WIT_BIND, help="CPU binding script"
        ),
...
class ContextBuilder(ABC):
    ...
    def build(
        ...
        if job_params is not None and input_params is None:
            ...
            if not any(
                k in job_params for k in ["config_start", "nodes", "time_limit"]
            ):
        ...
        job_values = self._apply_schema(
            resolved_job, job_schema_deduped, param_type="job", cast_to_str=False
        )
        input_values = self._apply_schema(
            resolved_input, input_schema_deduped, param_type="input", cast_to_str=True
        )

        # Let subclass build computed/special values
        computed_context = self._build_context(
            backend, ensemble_id, ensemble, physics, resolved_job, resolved_input
        )
    ...
    def _deduplicate_schema(
        self, schema: Optional[List[ContextParam]]
    ) -> List[ContextParam]:
        """Deduplicate schema params, keeping the LAST occurrence of each name.
...
class WitGPUContextBuilder(ContextBuilder):
    ...
    def _setup_wit_workdir(
        self, ensemble: Dict, job_params: Dict, subdir: str
    ) -> Tuple[Path, Path]:
    ...
    def _parse_geometry(
        self, physics: Dict, job_params: Dict
